...
\rev{Research on blood glucose prediction often feeds the current CGM readings and other factors into the physiological models to predict the short-term blood glucose levels (\eg in 30 minutes) to allow for precaution measures.}
...
In addition, \sysname investigates the feasibility of blood glucose monitoring without CGM devices for a longer time (\eg re-calibration per month), while previous work only shows the possibility for 3 hours~\cite{bib:JDST14:Sobel}.
}
