...
$U_{c}$ stands for the carbohydrate proportion of meals.
...
$U_{I}$ is the amount of insulin injected or simulated by the diabetes drugs, and $S^I$ and $bm$ refer to the insulin sensitive and body mass.
...
In \sysname, the window size of exercise is set to 24 hours, which optimizes the experimental results.
...
Blood glucose dynamic refers to the fluctuation of glucose mass $G_m$ in \equref{Eq:Gm} and glucose concentration $G(t)$ in \equref{Eq:G}.
...
\end{equation}
where $\delta_{abs}$ in \equref{Eq:abs} refers to the impact results of carbohydrate absorption, and $\delta_{egp}$ in \equref{Eq:egp} is the hepatic glucose production from the liver. 
Additionally, 
...
\end{equation}
where $\delta_{ind}$ in \equref{Eq:ind} describes results of insulin independent uptake, which is consumed by the central nervous system and the red blood cells.
$\delta_{dep}$ in \equref{Eq:dep} indicates the impact results of insulin dependent uptake. 
...
$\delta_{clr}$ in \equref{Eq:clr} stands for the influence of renal clearance on blood glucose. 
