...
\begin{figure}[t]
  \centering
  \includegraphics[width=1\columnwidth]{./img/pred_vs_gt2.pdf}
...
To demonstrate the effectiveness of the multi-task framework in making full use of the training dataset, we evaluation \modelname as follows.

\paragraph{Evaluation on the layers of framework}
To justify the effectiveness of the framework of \modelname, we study the results of inference by combining the grouped-input layer and the output layer on the Deep Dynamic RNN model(the middel layer). \figref{fig:cmp_model} shows the results. As we can see, the precision and recall of inference are growing up by adding each layer, with the increasements by 21.13\% in average precision and 18.57\% in average recall, respectively.

\begin{figure}[h]
  \centering
  \includegraphics[width=0.9\columnwidth]{./img/CMP_Models1.pdf}
  \caption{Effectiveness of Multi-task Framework.}
  \label{fig:cmp_model}
\end{figure}
...
\begin{itemize}
  ...
  The data of users belonging to a same group are trained together, and three different models are trained for the three groups (\ie healthy, Type I and Type II) respectively. The result of inference enables to model the similarities among a group but ignoring the individual personalities.
