...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.8\columnwidth]{./img/experiment_case2.pdf}
...
\subsection{Overview of System Performance}
\subsubsection{Result of Inference Overview}
...
In particular, the recalls for Level 1 (low blood glucose) and Level 4 (high blood glucose) are 83.13\% and 85.23\%, even though the training data for Level 1 and Level 4 only account for 9.88\% and 20.72\% of the entire training set.\textcolor[rgb]{1.00,0.00,0.00}{ It demonstrates \sysname can still be sensitive to the low/high blood levels under the limited data.
Overall, \sysname yields an accuracy of 82.14\%, showing the outstanding performance to track the blood glucose.}
...
\subsubsection{ Result of Inference Analysis}
\label{subsec:predict_result_analysis}
We detail the inference result based on the Clarke Error Grid Analysis (CEGA) \cite{bib:clarke2005original}.  The inference result are classified into four types from best to worst as shown in \tabref{predict_results}. Apparently, the percentages of result type decrease with from Type A (82.14\%) to Type E (0.91\%). In particular, the percentage of predicted errors (Type D and Type E), which probably leads to the serious troubles, are less than 5\%. The outstanding performance makes \sysname suitable for daily use.
...
Type A                  & \begin{tabular}[c]{@{}l@{}}The inference value is consistent with the true value.\\ (\emph{i.e.}, the inference blood glucose level is correct.)\end{tabular}                                                                                             & 82.14\%             \\ \hline
Type B                  & \begin{tabular}[c]{@{}l@{}}The inference result would not lead to inappropriate treatment. \\ (\emph{i.e.}, Level 2 is predicted as Level 3, or vice-versa.)\end{tabular}                                                                                 & 7.67\%              \\ \hline
...
\begin{figure}[t]
  ...
  \caption{The comparison of prediction and the ground truths of users.}
  \label{fig:pre_gt}
\end{figure}


\subsubsection{Details of Inference Analysis}
...
Although the predicted errors sometimes occur, most of the predict results is consistent with the true blood glucose levels. Specifically,  the dynamics of true blood glucose followed by the external factors \eg food intake (Non-diabetic user at 5:50, 11:30 and 19:00; Type I diabetic user at 6:00 and 16:50; Type II diabetic user at 6:00, 12:50 and 17:45), insulin injection ( Type I diabetic user at 7:30), drug intake (Type II diabetic use at 15:10) and exercises (Type II diabetic user at 15:30) are well matched by the predicted results.
...
To demonstrate the effectiveness of the multi-task framework in making full use of the training dataset, we evaluation \modelname from two views.
\begin{figure}[h]
  \centering
  ...
  \label{fig:cmp_model}
\end{figure}
\paragraph{Evaluation on the layer combination}
\textcolor[rgb]{1.00,0.00,0.00}{To justify the effectiveness of the framework of \modelname, we study the results of inference by combining the grouped-input layer and the output layer on the Deep Dynamic RNN model(the middel layer). \figref{fig:cmp_model} shows the results. As we can see, the precision and recall of inference are growing up by adding each layer, with the increasements by 21.13\% in average precision and 18.57\% in average recall, respectively. Moreover, the standard deviations narrow down gradually from 17.25\% to 10.25\%  of average precision, and from 20.75\% to 10.75\% of average recall. It demonstrates that the combination of three layers can well model the similarities of users in a same group as well as the distinct personality of every user.}


\begin{figure}[h]
  \centering
  \includegraphics[width=0.9\columnwidth]{./img/performance_of_multi_division.pdf}
  \caption{Performance of multi-division learning.}
  \label{fig:cmp_multi_division}
\end{figure}



\paragraph{Evaluation on the multi-division learning}
To evaluate the effectiveness of the multi-division learning, which shares data and knowledge between groups and users, we compare the performance of \modelname with the results of following models.

\begin{itemize}
  \item \emph{General Learning.}
  All the training data are directly fed into the model (\ie RNN) for training indifferently.
...
\textcolor[rgb]{1.00,0.00,0.00}{ The data of users belonging to a same group are fed into a model (\ie RNN) for training together. Three models of three groups (\ie healthy, Type I and Type II) are obtained finally. The group learning results in a \emph{group} model that sharing the general characteristics of users in a same group but not the users of other groups.}
  
  \item \emph{Single Learning.}
  We train a different model (\ie RNN) for each individual participant by feeding his/her own measurements into the model.
...
\textcolor[rgb]{1.00,0.00,0.00}{As shown, our multi-divisional learning framework (\sysname) outperforms best among the four learning approaches with 80.75\% of average precision and 82.57\% of average recall, and its standard deviations (17.18\% of average precision and 17\% of average recall) of four levels are also lowest, which well demonstrates the effectiveness and  stability of our multi-division learning. }

\textcolor[rgb]{1.00,0.00,0.00}{Compared to \modelname, although general learning method makes full use of the training data equally, it ignores the personal difference of blood glucose and thus leads to low inference results. Conversely, single learning approach encodes the individual characteristics but suffers from lacking of user-specific training dataset. Even though the similarities of users can be learnt by the group learning model, the inter-person physiological differences and data limitation still drop down the performance.}
...
\textcolor[rgb]{1.00,0.00,0.00}{\modelname combines the advantages of these three learning approaches, which  makes better use of the limited training data by sharing measurements among users and,  while preserving user-specific characteristics via the personal learning layer.}
...
\figref{fig:cmp_models} illustrates the results. Apparently, \modelname achieves best performance on both precisions and recalls. 
\begin{figure}[h]
  \centering
  \includegraphics[width=0.9\columnwidth]{./img/Model_CMP.pdf}
  \caption{Comparison with classic models.}
...
\textcolor[rgb]{0.00,0.59,0.00}{Since our dataset consists of personal measurements from 6 to 30 days (see \tabref{tab:bgdata}), we increase the individual training samples from 5 to 25 days by every 5 days and take the testing by the samples from remaining days. When the samples of a user are exhausted for training, we ignores this user for testing. For example, if the measurements of the CGM last for 7 days, and the requirement of the training data is 5 days. We take the data of the former 5 days for training and take the left for testing. However, while the requirement of training days increases to 10 days, we use all the user's data for training and ignore him/her for testing.}
